...
export type NewsTag = "toefl" | "ielts" | "gre" | "det" | "toeic";
...
type NewsCacheEntry = {
  time: number;
  items: NewsItem[];
  refreshPromise: Promise<NewsItem[]> | null;
};
...
export function clampNewsLimit(value: string | number | null | undefined) {
  const parsed = Number(value ?? 6);
  if (!Number.isFinite(parsed)) return 6;
  return Math.min(Math.max(Math.trunc(parsed), 1), 12);
}

export function normalizeNewsTag(value: string | null | undefined): NewsTag | "all" {
...
function cacheKey(tag: NewsTag | "all", limit: number) {
  return `${tag}:${limit}`;
}

function buildFallbackItems(tag: NewsTag | "all", limit: number) {
  const fallback =
    tag === "all"
...
async function fetchFreshNews(tag: NewsTag | "all", limit: number) {
  ...
  if (!items.length) {
    items = buildFallbackItems(tag, limit);
  }

  return items;
}

function refreshNewsFeed(tag: NewsTag | "all", limit: number) {
  const key = cacheKey(tag, limit);
  ...
  const refreshPromise = fetchFreshNews(tag, limit)
    .then((items) => {
      cache.set(key, {
        time: Date.now(),
        items,
    ...
    .catch(() => {
      cache.set(key, {
        time: current?.time ?? 0,
        items: current?.items ?? buildFallbackItems(tag, limit),
        refreshPromise: null,
      });
      return current?.items ?? buildFallbackItems(tag, limit);
    });
...
export async function readNewsFeed(
...
): Promise<NewsFeedState> {
  const key = cacheKey(tag, limit);
  const hit = cache.get(key);
  const allowStale = options?.allowStale !== false;

  if (hit?.items?.length && Date.now() - hit.time < CACHE_TTL) {
    return {
      items: hit.items,
      cached: true,
      stale: false,
  ...
  if (allowStale) {
    if (hit?.items?.length) {
      void refreshNewsFeed(tag, limit);
      return {
        items: hit.items,
        cached: true,
        stale: true,
    ...
    const fallbackItems = buildFallbackItems(tag, limit);
    cache.set(key, {
      time: 0,
      items: fallbackItems,
      refreshPromise: null,
  ...
  const items = await refreshNewsFeed(tag, limit);
  return {
    items,
    cached: false,
    stale: false,
